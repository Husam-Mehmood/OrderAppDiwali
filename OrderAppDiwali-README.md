...
---
### Housekeeping suggestions (not yet done)
- Add a `.gitignore` for `out/` and `.idea/` — compiled `.class` files and IDE config are currently committed to the repo.
- Add a `LICENSE` file (e.g. MIT) if you're comfortable with others reusing/learning from the code.
